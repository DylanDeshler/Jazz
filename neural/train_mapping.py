...
def get_batch(split='train', batch_size=batch_size):
    ...
    Y = np.memmap(f'/home/ubuntu/Data/low_measures_large_actions_256top5_64.bin', dtype=np.float16, mode='r', shape=(4403211, 768))
    ...
    if split == 'train':
        ...
        idxs = torch.randint(int(len(X) * 0.98), len(X) - n_chunks, (batch_size,))
    x = torch.from_numpy(np.stack([X[idx:idx+n_chunks] for idx in idxs], axis=0)).pin_memory().to(device, non_blocking=True)
    y = torch.from_numpy(np.stack([Y[idx:idx+n_chunks] for idx in idxs], axis=0)).pin_memory().to(device, non_blocking=True)
...
def estimate_loss():
    ...
    for i, split in enumerate(['train', 'val']):
        ...
        for k in tqdm(range(eval_iters)):
            X, Y = get_batch(split, batch_size=batch_size * gradient_accumulation_steps)
            with ctx:
                loss, _ = model(X, Y)
            losses[k] = loss.item()
