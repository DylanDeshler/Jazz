...
from diffusion_forcing import DiT_B as net
...
# I/O
out_dir = 'DiT_measures_bpm_B_16_6'
eval_interval = 5000
...
model_args = dict(in_channels=vae_embed_dim, levels=levels, spatial_window=spatial_window, temporal_window=temporal_window,  num_actions=math.prod(levels))
...
def estimate_loss(is_causal):
    ...
    for i, split in enumerate(['train', 'val']):
        ...
        for k in tqdm(range(eval_iters * gradient_accumulation_steps)):
            ...
            with ctx:
                loss, _ = model(X, bpm, actions, is_causal=is_causal)
            losses[k] = loss.item()
