...
tokenizer2 = load_model(os.path.join('tokenizer_low_large_24576', 'ckpt.pt'), Tokenizer)
...
idxs = np.random.choice(np.arange(len(measure_paths)), size=16, replace=False)#256
n_steps = 32
batch_size = 4
    ...
    for idx in tqdm(idxs):
        ...
        if not EVAL_ITERATIVE:
            ...
            with ctx:
                ...
                y5 = base2_dit.generate(gen_shape, n_steps=n_steps, noise=gen_noise)
                y5 = y5.transpose(2, 3).view(batch_size * n_chunks, vae_embed_dim, spatial_window)
                y5 = tokenizer2.decode(y5, shape=(1, 24576), n_steps=n_steps, noise=None)
        ...
        if idx % 4 == 0:
            ...
            sf.write(
                ...
                data=y5[to_sample].flatten().cpu().detach().numpy(), 
                samplerate=rate,
                subtype='PCM_16'
            )
            # sf.write(
...
if not EVAL_ITERATIVE:
    ...
    # Measure (Moving Average BPM) -> Real Samples FAD:  79.57578327725473
    # Measure (Median BPM) -> Real Samples FAD:  81.30687320329737
    ...
    ## 32 chunks 130k steps
    # Base -> Real Samples FAD:  54.98616289571552
    # Measure (Global BPM) -> Real Samples FAD:  47.627474487065086
    # Measure (Moving Average BPM) -> Real Samples FAD:  45.86883719134492
    # Measure (Median BPM) -> Real Samples FAD:  45.156782070490436
