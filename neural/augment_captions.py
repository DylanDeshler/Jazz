...
def build_generation_prompt(caption):
    return (
        ...
        "CRITICAL RULES:\n"
        "1. NEVER copy the reference caption verbatim.\n"
        "2. EVERY variation must use different phrasing, word orders, or sentence structures.\n"
        "3. DO NOT add any information not explicitly in the refernce.\n"
        "4. DO NOT SUMMARIZE. Every variation must retain the descriptive detail, mood, instruments, historical context, and stylistic adjectives found in the reference.\n"
        "5. ABSOLUTE CONSTRAINTS. If the reference contains a BPM or Key, you MUST preserve them in every single variation while varying their presentation.\n"
        "6. MATCH THE LENGTH. Keep the overall length and core attributes identical, but change the syntax drastically.\n\n"
        "7. ROTATE CLAUSES. Achieve diversity by changing the sentence structures, moving technical details around, or flipping the order of descriptions.\n"
        "EXAMPLE:\n"
...
def build_selection_prompt(reference, pool_list):
    ...
    return (
        ...
        "4. HALLUCINATION CHECK: You must aggressively reject any candidate that introduces new musical genres, acoustic characteristics, instruments or anything else not present in the Reference Caption (e.g., injecting 'jazz' or 'polka' into a 'ragtime' prompt)."
