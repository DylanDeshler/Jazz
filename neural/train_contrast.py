...
def sample_non_overlapping(data, start_fraction, end_fraction):
    pos = np.random.randint(np.arange(low=int(len(file_offsets) * start_fraction), high=int(len(file_offsets) * end_fraction)), size=(batch_size // 2, ))
...
def get_batch(split='train', batch_size=batch_size):
    ...
    if split == 'train':
        idxs = sample_non_overlapping(data, 0, 0.98)
    else:
