...
# I/O
out_dir = 'tokenizer_high_tokens1'
eval_interval = 1000
log_interval = 100
...
def get_batch(split='train'):
    if split == 'train':
        ...
        batch = torch.from_numpy(np.stack([data[idx:idx+max_seq_len] for idx in idxs], axis=0)).permute(0, 2, 1).pin_memory().to(device, non_blocking=True)
        return batch
...
model_args = dict(z_shape=(256, 50), n_residual_layers=3, lstm=0, transformer=1, down_proj=768, in_channels=128, dimension=256, n_filters=128, ratios=[1, 2, 2, 1], dilation_base=2, c0=192, c1=384, c2=768, timescale=400)
