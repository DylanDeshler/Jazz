...
def get_batch(split='train'):
    ...
    audio = torch.from_numpy([data[idx:idx+max_samples] for idx in np.arange(batch_size)]).unsqueeze(1).pin_memory().to(device, non_blocking=True)
...
model_args = dict(z_shape=(16, 32), in_dim=1, hidden_dim=512, latent_dim=16, n_heads=8, depth=4, n_interleave=4, n_latents=32, kernel_size=7)
