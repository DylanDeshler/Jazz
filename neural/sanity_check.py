...
def get_batch(split='train'):
    if split == 'train':
        ...
        x = torch.from_numpy(np.stack([np.stack([data[0+i*spatial_window:0+(i+1)*spatial_window] for i in range(temporal_window)], axis=0) for idx in idxs], axis=0)).pin_memory().to(device, non_blocking=True)
        return x
