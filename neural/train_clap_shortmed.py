"""
Train a CLAP-style audio<->text model on the jazz dataset -- SHORT+MEDIUM caption
variant (Option A: filter out long captions on the TRAIN split only).

Hypothesis: long captions are so descriptive they act as near-unique per-song
fingerprints, letting the text tower memorize a caption->song lookup (the same
overfitting trap the audio-only contrastive model avoided). Dropping the long
tier from training removes that signal while keeping short+medium.

Mechanics (see _read_batch_cpu): the text memmap rows are pre-shuffled, and each
row's tier (0=short, 1=medium, 2=long) is recoverable from shuffled_indices +
orig_sub_shape WITHOUT reading the memmap. So we over-read a contiguous slab,
compute tiers in RAM for free, keep only the allowed tiers, and index them out of
the in-memory slab -- the single contiguous read (the thing the prefetcher made
cheap) is preserved; we just read ~1.5x the bytes and drop 1/3 before the GPU.

The filter applies to TRAIN batches only. Val stays unfiltered so the per-tier
retrieval breakdown still evaluates long captions as a held-out generalization
probe (train short+medium, watch whether long-caption retrieval tracks/diverges).
...
Single GPU:
    $ python train_clap_shortmed.py
DDP (4 gpus):
    $ torchrun --standalone --nproc_per_node=4 train_clap_shortmed.py
"""
...
# caption tiers to TRAIN on (0=short, 1=medium, 2=long). Long is dropped here to
# test whether over-descriptive captions drive the text-side memorization. Val is
# never filtered, so eval still reports all three tiers. Set to (0,1,2) to disable
# filtering (then this script behaves identically to train_clap.py).
train_tiers = (0, 1)
...
config['train_tiers'] = str(train_tiers)   # tuple isn't captured above; log it explicitly
# -----------------------------------------------------------------------------
...
# tier-filter constants (train split only). fraction kept drives how much larger a
# contiguous slab we over-read to still net `batch_size` allowed rows.
_TRAIN_TIERS = set(int(t) for t in train_tiers)
_FILTER_TRAIN = len(_TRAIN_TIERS) < 3
_KEEP_FRAC = len(_TRAIN_TIERS) / 3.0
...
def _pick_window(split, batch_size):
    """Choose a contiguous row window and the LOCAL indices to keep from it.

    Returns (start, keep_local, bounds, song_ids, tiers) where keep_local indexes
    into the [start, start+window) slab. Tier filtering (train split only) is done
    here from shuffled_indices alone -- NO memmap read -- so an under-filled window
    is simply re-drawn for free. Val (or train_tiers==all) => contiguous, no drop."""
    n_rows = N_TRAIN_ROWS if split == 'train' else N_VAL_ROWS

    if not (split == 'train' and _FILTER_TRAIN):
        start = int(np.random.randint(n_rows - batch_size))
        bounds, song_ids, tiers = map_rows(start, split, batch_size)
        return start, np.arange(batch_size), bounds, song_ids, tiers

    # over-read ~1/keep_frac rows (+30% margin) so the kept subset reaches batch_size
    window = min(n_rows, int(math.ceil(batch_size / _KEEP_FRAC * 1.3)))
    allowed = list(_TRAIN_TIERS)
    for _attempt in range(100):
        start = int(np.random.randint(n_rows - window))
        bounds_w, song_ids_w, tiers_w = map_rows(start, split, window)
        keep_local = np.where(np.isin(tiers_w, allowed))[0]
        if len(keep_local) >= batch_size:
            keep_local = keep_local[:batch_size]
            return start, keep_local, bounds_w[keep_local], song_ids_w[keep_local], tiers_w[keep_local]
        # window under-filled (statistically shouldn't happen at 1.3x margin) --
        # re-draw a new window. this is index math only, no disk read.
        print(f"[tier-filter] re-draw: window@{start} yielded only {len(keep_local)}/"
              f"{batch_size} allowed rows (attempt {_attempt + 1})")
    raise RuntimeError(
        f"tier filter could not fill a batch of {batch_size} from a {window}-row "
        f"window after 100 tries (allowed tiers={allowed}); raise the window margin.")
...
def _read_batch_cpu(split='train', batch_size=batch_size):
    ...
    Returns a dict of pinned CPU tensors plus (rows, tiers). Deliberately does
    ...
    derive from the SAME window + keep_local, so the tuple is atomic -- row i of
    each always describes the same (caption, song) pair; the linkage cannot be
    broken by prefetch (or by tier filtering) as long as this dict travels together.

    Tier filtering (train split): the whole [start, start+window) slab is read in
    ONE contiguous memmap access, then the kept rows are fancy-indexed out of it in
    RAM -- the contiguous read the prefetcher relies on is preserved; only ~1.5x
    the bytes are read and 1/3 are discarded before the GPU."""
    if split == 'train':
    ...
    song_paths_list = train_raw_paths if split == 'train' else val_raw_paths

    start, keep_local, bounds, song_ids, tiers = _pick_window(split, batch_size)
    window = int(keep_local[-1]) + 1 if len(keep_local) else batch_size
    rows = start + keep_local              # global row indices of the kept captions
    ...
    # One contiguous read of the (possibly over-sized) slab, then index kept rows.
    slab = np.ascontiguousarray(text_mm[start:start + window])
    text_np = slab if len(keep_local) == window and keep_local[0] == 0 else slab[keep_local]
    text = torch.from_numpy(np.ascontiguousarray(text_np)).pin_memory()
    if profile:
    ...
    # crop each (only for the KEPT rows). to_device=False -> pinned CPU (no CUDA).
    ...
    return {
        ...
        'rows': rows,     # global row indices, for get_text_from_index during eval
    }
...
def _to_device_batch(cpu, return_start=False):
    ...
    if return_start:
        # returns the kept rows' GLOBAL indices (not a scalar start): after tier
        # filtering the kept rows aren't contiguous, so callers must map through
        # these to recover the right caption (see save_retrieval_samples).
        tiers = cpu['tiers'].to(device, non_blocking=True)
        return (text, audio, text_mask, audio_mask, song_ids), cpu['rows'], tiers
...
class Prefetcher:
    ...
    always intact regardless of prefetch depth or tier filtering."""
...
def retrieval_metrics(split='val'):
    ...
    NOTE: called on 'val', which is NEVER tier-filtered, so the long tier is still
    present here -- the per-tier long R@k is the held-out generalization probe for
    the short+medium training regime.
    """
...
def save_retrieval_samples(step, split='val', n_show=8, pool=512):
    ...
    model.eval()
    (text, audio, text_mask, audio_mask, song_ids), rows, _ = get_batch(split, batch_size=min(pool, batch_size), return_start=True)
    with ctx:
        enc_a = model.module.encode_audio if ddp else model.encode_audio
        enc_t = model.module.encode_text if ddp else model.encode_text
    ...
    # rows are the kept captions' GLOBAL indices (post-filter, not contiguous)
    captions = [get_text_from_index(int(rows[i]), split) for i in range(t.shape[0])]
    lines = []
...
if wandb_log and master_process:
    import wandb
    if init_from == 'resume':
        wandb.init(project=wandb_project, name=wandb_run_name, resume='allow', config=config)
    else:
