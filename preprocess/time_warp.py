...
def generate_audio_measures(paths):
    ...
    assert (len(downbeat_indices) - 1) * TARGET_SAMPLES == len(np.concatenate(audios, axis=0))
    sf.write(
        file=out_path, 
...
def time_warp_measures():
    ...
    beat_paths = sorted(glob.glob('/home/dylan.d/research/music/Jazz/jazz_data_16000_full_clean_beats/*.beats'))[:64*10]
    # import json
