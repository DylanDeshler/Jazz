...
### Samples
I've included a few samples generated from the final model towards the end of its training below. Please be aware that the dataset contains recordings from 1880-1980 and the model has learned to mimic that quality, do not expect modern high fidelity studio quality!

| Generation |
|----------|
| <audio controls><source src="samples/stage2/2_cfg.wav" type="audio/wav"></audio> |
| <audio controls><source src="samples/stage2/6_cfg.wav" type="audio/wav"></audio> |
| <audio controls><source src="samples/stage2/5_cfg.wav" type="audio/wav"></audio> |
| <audio controls><source src="samples/stage2/7_cfg.wav" type="audio/wav"></audio> |
| <audio controls><source src="samples/stage2/1_cfg.wav" type="audio/wav"></audio> |
...
It is worth underscoring the training data's poor quality. Many of these songs were recorded with noisy equipment. Further resampling to 16KHz wrosens the quality, but at a tradeoff that I found acceptable for an academic project. While this approach improves the overall data quality, at this scale just having more data is more important than having less but higher quality data. To that end, when doing my final run of end-to-end training I used all of the data available. Further because of the latent action modeling, it will become possible to avoid generating noisy samples at inference.
...
### Continuous Actions
![Style Tokens](style_tokens.png "Style Tokens")
...
# Instruments
JazzSet has song level instrument annotations, but to build a generative model conditioned on instruments we require fine grained annotaitons. I followed the mulitple instance learning (MIL) approach from computer vision to produce the annotations. MIL trains a model to predict the global labels aggregated from local predictions, forcing the model to learn some . Inspired by DETR I augmented the architecture with learnable queries for each instrument class and cross-attend them with the features to produce "frame" logits. The final global prediction is a weighted average using these logits. Once this model has been trained we use it to annotate the entire dataset with fine grained instrument labels. Unfortunately this labels are pretty noisy because of the training process and dataset. The distribution of labels is very unbalanced so the MIL model learns some good and some bad features. After labeling the dataset we can visualize the probabilities for each class in a histogram and fit a GMM to the distribution to help pinpoint the positive, negative, and uncertain threshold. Then I train a U-Net on these noisy labels, the outputs from this model are reasonably good and used to produce the instrument conditioning for the generative model.
...
Below are a few cherry picked samples that use actions from real songs as conditioning. Due to the low sample rate and inherently poor audio quality of the original recordings, the peak audio quality and fidelity of these generations is lacking, but I find the range, controlability, and musical quality to be impressive.
...
# Future Work
Scale! Scale! Scale! I am very interested to see what action sets could be learned across a much larger and more diverse dataset. I would also like to have finegrained instrument annotations to condition the action model on to make actions more general and seperate from instruments. Improving generation quality by model and data scaling and hyperparameter tuning. Increasing the maximum context length to support generation of more musical structure. Learning a mapping from actions to text to support localized editing with text. Full resolution (44khz). Learned time warping. Potentially learning a tiny discrete set of ransitions between measures. 
